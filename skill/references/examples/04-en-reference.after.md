...
- `options.sanitize` — boolean — strip raw HTML from the output. Default true.
  Setting this to false allows arbitrary HTML through, which is a security risk if the input is user-supplied, so only disable it for trusted content such as your own documentation files.
